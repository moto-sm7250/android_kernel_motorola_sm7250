...
#define AWINIC_CALI_FILE  "/mnt/vendor/persist/xxxx/audio/aw_cali.bin"
...
static int aw882xx_attr_cali_start_up(int32_t *cali_re, int32_t *f0)
{
	int ret;
	struct cali_cfg set_cfg, store_cfg;
	struct cali_data cali_data;
	...
	*cali_re = cali_data.data[0];
	...
	/*start white noise*/
	ret = aw_send_afe_cal_apr(AFE_PARAM_ID_AWDSP_RX_NOISE_L,
				(void *)&noise_enable, sizeof(int32_t), true);
	if (ret) {
		pr_err("%s: set noise enable failed", __func__);
		goto f0_failed;
	}
	...
	/*get f0 value*/
	ret = aw_send_afe_cal_apr(AFE_PARAM_ID_AWDSP_RX_F0_L,
				(void *)&local_f0, sizeof(int32_t), false);
	if (ret) {
		pr_err("%s: read f0 failed ! \n", __func__);
		*f0 = 0;
		goto f0_failed;
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strcmp("start_cali", buf)) {
		pr_err("%s: not define cmd %s \n", __func__, buf);
		return 0;
	}

	ret = aw882xx_attr_cali_start_up(&local_re, &local_f0);
	if (ret) {
		pr_err("%s: cali failed \n", __func__);
		return 0;
	}

	aw882xx->cali_re = local_re;
	aw882xx->cali_f0 = local_f0;
	return count;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	len = snprintf(buf, PAGE_SIZE, "%d", aw882xx->cali_re);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret < 0) {
		pr_err("%s, read buf %s failed\n", __func__, buf);
		return ret;
	}
	aw882xx->cali_re = data[0];
	return count;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	len = snprintf(buf, PAGE_SIZE, "%d", aw882xx->cali_f0);
